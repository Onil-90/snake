...
class Game():
    def __init__(self, size=[30,30], size_square=20, len=4, speed=1):
        ...
        self.target = [random.choice(range(self.size[1])), random.choice(range(self.size[0]))]
    ...
    def draw(self):
        ...
        if not self.game_over:
            for _, [x,y] in enumerate(self.body):
                RECT = pg.rect.Rect((x*self.size_square,y*self.size_square, self.size_square, self.size_square))
                pg.draw.rect(self.screen, GREEN, RECT)
            # draw target
            TARGET = pg.rect.Rect((self.target[0]*self.size_square, self.target[1]*self.size_square, self.size_square,self.size_square))
